<script lang="ts">
	...
	let images: { id: number; file: string }[] = [];
	...
	{#if showImage}
		{#each images as item (item.id)}
			<div class="relative">
				...
				<div
					class="rounded-full bordered border-2 border-red-500 absolute top-2 right-4 cursor-pointer shadow-lg"
				>
					<button on:click={() => deleteThis(item.id)}
						><span class=" text-xs hover:text-sm font-bold p-2 text-red-500">X</span></button
					>
				</div>
			</div>
		{/each}
