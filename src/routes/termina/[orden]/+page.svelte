	...
	{#if order}
		<div class="flex flex-col mx-auto md:ml-5 p-5 w-11/12 md:w-2/3 md:border m-2">
			<div class="text-center font-semibold bg-gray-100">
				Este es tu numero de order de compra: <span class="font-bold" />
				{order.id}
			</div>
			<h3>{order.usuario.name}</h3>
			<p>
				Direccion de Entrega: {order.direccion_entrega}
				{order.ciudadEnt}, {order.departamento_ent}
			</p>

			<p>Teléfono: {order.usuario.phone}</p>
			<p>Email: {order.usuario.email}</p>
		</div>
		...
		<div class="p-5  mx-5">
			<div class="overflow-auto rounded-lg shadow hidden md:block">
				<table class="w-full">
					...
					<tbody class="divide-y divide-gray-100">
						{#each order.detalle as producto, i}
							<tr class:bg-white={i % 2 === 0} class:bg-gray-50={i % 2 !== 0}>
								<td class="p-3 text-sm text-gray-700 whitespace-nowrap">
									{producto.product.name}
								</td>
								<td class="p-3 text-sm text-gray-700 whitespace-nowrap">
									{(producto.precio/(1+producto.product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
								</td>
								<td class="p-3 text-sm text-gray-700 whitespace-nowrap">
									{((producto.precio/(1+producto.product.tax/100))*(producto.product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
								</td>
								<td class="p-3 text-sm text-gray-700 whitespace-nowrap"
									>{producto.cantidad}</td
								>
								<td class="p-3 text-sm text-gray-700 whitespace-nowrap"
									>{(producto.precio * producto.cantidad).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}</td
								>
							</tr>
						{/each}
						<tr>
							...
							<td class="p-3 text-sm text-gray-700 whitespace-nowrap font-semibold"
								>{order.valor.toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}</td
							>
						</tr>
					</tbody>
				</table>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
				{#each order.detalle as producto, i}
					<div class="bg-white space-y-3 p-4 rounded-lg shadow-md">
						<div class="text-sm text-primary text-center">{producto.product.name}</div>
						<div class="text-sm font-medium text-black">
							Precio: {(producto.precio/(1+producto.product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
						</div>
						<div class="text-sm font-medium text-black">
							Iva: {((producto.precio/(1+producto.product.tax/100))*(producto.product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
						</div>
						<div class="text-sm font-medium text-black">
			...
			<div class="flex md:hidden my-5 ">
				<span class="font-semibold mr-1">El valor total de tu order es de: </span> {order.valor.toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
			</div>
		</div>
	{:else}
		<p>Numero de order no existe</p>
	{/if}
{/await}
