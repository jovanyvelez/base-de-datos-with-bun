<script lang="ts">
	...
	$: $cart,
		(resum = {
			total: $cart.reduce(
				(a, c) => a + c.qtyBuy * c.Price[0].price1 * (1 - discount / 100)*(1+c.tax/100),
				0
			),
			items: $cart.reduce((a, c) => a + c.qtyBuy, 0),
			iva: $cart.reduce(
				(a, c) => a + c.qtyBuy * c.Price[0].price1 *(1 - discount / 100)*(c.tax/100),
				0
			),
			subtotal: $cart.reduce(
				(a, c) => a + c.qtyBuy * c.Price[0].price1 * (1 - discount / 100),
				0
			),
		});
...
<div class="grid grid-cols-1 sm:grid-cols-6 mx-5 my-5">
	...
	{#each $cart as product (product.id)}
		...
		<div class="flex justify-center items-center">
			{(product.Price[0].price1).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
		</div>
		<div class="flex justify-center items-center">
			{(product.Price[0].price1*(discount/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
		</div>
		<div class="flex justify-center items-center">
			{(product.Price[0].price1*(1-discount/100)*(product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
		</div>
		<div class="flex flex-col justify-center items-center">
			<ButonQuantity {product} />
		</div>
		<div class="flex justify-center items-center">
			{(product.Price[0].price1*(1-discount/100)*(1+product.tax/100)*product.qtyBuy).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
		</div>
